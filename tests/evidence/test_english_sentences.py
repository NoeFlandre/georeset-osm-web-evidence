...
class EnglishSentenceCandidateTests(unittest.TestCase):
    def test_english_sentence_quota_is_satisfied_after_quality_metadata(self):
        page_rows = []
        sentence_terms = [
            "oak",
            "pine",
            ...
            page_rows.append(
                {
                    "osm_type": "way",
                    "osm_id": 1,
                    "polygon_name": "Forest 1",
                    "has_wikipedia_articles": False,
                    "url": f"https://example.org/1/{url_index}",
                    "final_url": f"https://example.org/1/{url_index}",
                    ...
                    "query_language": "en",
                    "text": (
                        f"The northern {sentence_term} forest contains "
                        "wetlands and protected wildlife habitats."
                    ),
                    "query_local_language": "en",
                }
            )
        page_text_df = pd.DataFrame(page_rows)
        pilot_df = pd.DataFrame(
            [{"osm_type": "way", "osm_id": 1, "world_region": "Europe"}]
        )

        result = english_sentence_quota_is_satisfied(
            page_text_df,
            pilot_df,
